...
What is executable and falsifiable: webhook HMAC validation, attacked 42 ways in tests; replay/order controls; deterministic authority attenuation; guardrails; provider-call accounting; abstention; timeout handling; audit verification; and the independent checker. The baseline suite currently contains 297 tests and a 14/14 mutation check; the final verification record must quote the final frozen run rather than assume those numbers.
