...
This layer strengthens MandateGuard without changing the frozen nine-arm benchmark or adding to its pytest suite.
...
`scripts/release_check.sh` invokes this after the original frozen checks. It is deliberately outside the benchmark pytest suite count, just like RecoveryTruth.
