...
def generate_report() -> Path:
    ...
    lines = [
        "# MandateGuard: recovery results and decision evidence",
        "",
        ...
        "",
        "When a scheduled UPI AutoPay payment fails, how much recovery does a policy achieve, what recoverable value does it forgo, and does it execute prohibited actions? Every policy sees the same frozen outcomes. This is a controlled synthetic comparison, not a production trial.",
        "",
        ...
        "",
        "The fully guarded B3 workflow is shown below for inspection, not as an unconditional recommendation. Read the complete price sweep alongside it: the economic ordering changes with the assumed cost of prohibited execution.",
        "",
        "![Recovery and prohibited execution](frontier.png)",
        "![Complete price sweep](sensitivity.png)",
        "",
        "Means per batch, in simulated INR. The interpreter difference is B3 minus B2 under the same controls, using the offline deterministic interpreter stub. It is not measured live-model uplift.",
        "",
        "| Failure mix | B3 recovered | Recoverable value forgone | Prohibited value executed | Interpreter recovery difference |",
        "|---|---:|---:|---:|---:|",
    ]
    by_key = {(row["regime"], row["arm"]): row for row in aggregates}
    for regime in manifest["regimes"]:
        b2, b3 = by_key[(regime, "B2")], by_key[(regime, "B3")]
        lines.append(
            f"| {regime} | {money(_mean(b3, 'incremental_recovered_inr'))} | "
            f"{money(_mean(b3, 'legitimate_recovery_forgone_inr'))} | "
            f"{money(_mean(b3, 'realized_harm_inr'))} | "
            f"{money(_mean(b3, 'incremental_recovered_inr') - _mean(b2, 'incremental_recovered_inr'))} |"
        )
    lines.extend([
        "",
        "**Interpretation:** controls can prevent prohibited execution while sacrificing legitimate recovery. A higher recovery figure alone is not evidence of a better policy. The offline interpreter comparison tests a simulated policy mechanism; the separately saved real-model call establishes integration only.",
        "",
        "**Evidence boundary:** the batch uses a local provider simulator. Saved Razorpay Test Mode capture is a separate Payment Link fallback example; a created link is not recovered money. Neither establishes production AutoPay performance.",
        "",
        "## Full policy results",
        "",
        "<details>",
        "<summary>Inspect all policy arms and diagnostic metrics</summary>",
        "",
    ...
    lines.extend(
        [
            ...
            f"Policy version: `{manifest['version']}`",
            f"Ledger SHA-256: `{manifest['dataset_sha256']}`",
            f"Rules SHA-256: `{manifest['rules_sha256']}`",
            f"Output bundle SHA-256: `{manifest['outputs_sha256']}`",
            f"Seeds: `{', '.join(map(str, manifest['seeds']))}`",
            "",
            "Reproduce from the repository root with `bash scripts/evaluate.sh`. This report is generated by `bailiff/report.py` from `outputs/manifest.json` and `outputs/aggregate.json`; do not hand-edit its numbers.",
        ]
    )
